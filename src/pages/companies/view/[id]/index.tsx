...
function CompanyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CompanyInterface>(
    ...
    () =>
      getCompanyById(id, {
        relations: ['user', 'inventory', 'product', 'warehouse'],
      }),
  );
              ...
              {hasAccess('inventory', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/inventories/create?company_id=${data?.id}`}>
              ...
              {hasAccess('warehouse', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/warehouses/create?company_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.warehouse?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/warehouses/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('warehouse', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/warehouses/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('warehouse', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    warehouseHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
