...
import { deleteWarehouseProductById } from 'apiSdk/warehouse-products';
...
function ProductViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProductInterface>(
    ...
    () =>
      getProductById(id, {
        relations: ['company', 'inventory', 'warehouse_product'],
      }),
  );
  ...
  const warehouse_productHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('inventory', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/inventories/create?product_id=${data?.id}`}>
              ...
              {hasAccess('warehouse_product', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Warehouse Products
                    </Text>
                    <NextLink passHref href={`/warehouse-products/create?product_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.warehouse_product?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/warehouse-products/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess(
                                ...
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/warehouse-products/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess(
                                ...
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    warehouse_productHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
