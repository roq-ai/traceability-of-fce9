...
function ProductListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProductInterface[]>(
    ...
    () =>
      getProducts({
        relations: ['company', 'inventory.count', 'warehouse_product.count'],
      }),
  );
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    <Td>{record.name}</Td>
                    <Td>{record.traceability_code}</Td>
                    ...
                    {hasAccess('warehouse_product', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.warehouse_product}</Td>
                    )}
                    <Td>
