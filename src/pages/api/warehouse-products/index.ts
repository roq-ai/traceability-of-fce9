...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'POST':
      return createWarehouseProduct();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getWarehouseProducts() {
    const data = await prisma.warehouse_product
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'warehouse_product'));
  ...
  async function createWarehouseProduct() {
    ...
    const data = await prisma.warehouse_product.create({
      data: body,
    });
    return res.status(200).json(data);
  }
